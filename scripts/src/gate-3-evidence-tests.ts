...
const clientEmail = process.env.GATE_3_CLIENT_EMAIL ?? "client@mustashark.com";
const clientPassword = process.env.GATE_3_CLIENT_PASSWORD ?? "Gate2RealAuth!2026";
const lawyerEmail = process.env.GATE_3_LAWYER_EMAIL ?? "lawyer@mustashark.com";
const lawyerPassword = process.env.GATE_3_LAWYER_PASSWORD ?? "Gate2RealAuth!2026";
...
async function login(email: string, password: string, role: "client" | "lawyer") {
  ...
  assert.equal(result.authProvider, "local", `auth provenance missing for ${email}: ${JSON.stringify(result)}`);
  assert.equal(typeof result.jwt, "string", `login JWT missing for ${email}`);
...
async function makeSettlementFixture(clientId: string, lawyerId: string, requestStatus: "approved" | "pending") {
  ...
  await db.insert(representationQuotesTable).values({
    id: quoteId, clientId, lawyerId, title: "Gate 3 evidence fixture", description: "ephemeral",
    totalAmount: "50.00", currency: "QAR", status: "funding", fundingMode: "per_stage", createdAt: now, updatedAt: now,
  });
  await db.insert(representationMilestonesTable).values({
    id: milestoneId, quoteId, stage: "stage_1", percentage: "100.00", amount: "50.00", title: "Gate 3 Stage", status: "under_review", createdAt: now, updatedAt: now,
  });
  ...
  await db.insert(escrowTransactionsTable).values({ id: crypto.randomUUID(), escrowAccountId: escrowId, milestoneId, type: "deposit", status: "posted", amount: "50.00", currency: "QAR", reference: "gate3-deposit", createdBy: clientId, createdAt: now });
  await db.insert(escrowTransactionsTable).values({ id: crypto.randomUUID(), escrowAccountId: escrowId, milestoneId, type: "stage_allocation", status: "posted", amount: "50.00", currency: "QAR", reference: "gate3-allocation", createdBy: clientId, createdAt: now });
  ...
  await db.insert(milestoneReleaseRequestsTable).values({
    id: requestId, milestoneId, proofId, clientId, lawyerId, status: requestStatus,
    reviewDeadlineAt: new Date(Date.now() + 3600000), decidedAt: requestStatus === "approved" ? now : null, createdAt: now, updatedAt: now,
  });
  return { quoteId, escrowId, milestoneId, proofId, requestId };
}

async function cleanup(f: Awaited<ReturnType<typeof makeSettlementFixture>>) {
...
assert(client && lawyer && admin, "canonical Gate 3 identities must exist");
