...
export function HandoverOptions({ sameCountry = true, availableModes, selected, onSelect }: { sameCountry?: boolean; availableModes?: HandoverMode[]; selected?: HandoverMode; onSelect?: (mode: HandoverMode) => void }) {
  const modes = availableModes ?? (sameCountry ? ["local", "office", "courier"] : ["international"]);
...
const styles = StyleSheet.create({
  wrap:{gap:9}, card:{flexDirection:"row",alignItems:"center",gap:11,borderWidth:1,borderColor:C.border,borderRadius:15,padding:13,backgroundColor:C.card},
  active:{borderColor:C.gold,backgroundColor:"#FBF7EA"}, icon:{width:40,height:40,borderRadius:12,backgroundColor:"#F8F1D9",alignItems:"center",justifyContent:"center"}, activeIcon:{backgroundColor:C.gold},
  copy:{flex:1,alignItems:"flex-end"}, title:{fontSize:12,fontFamily:"Inter_700Bold",color:C.foreground,textAlign:"right"}, description:{fontSize:9,color:C.mutedForeground,fontFamily:"Inter_400Regular",marginTop:3,textAlign:"right"},
});
