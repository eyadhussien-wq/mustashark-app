...
export default function PaymentProofScreen() {
  ...
  const currency = useMemo(() => getCurrency(user?.country ?? "qatar"), [user?.country]);
  const confirmedTotal = useMemo(
    () => proofs.filter((p) => p.status === "confirmed").reduce((sum, p) => sum + Number(p.amount), 0),
    [proofs],
  );
  const pendingTotal = useMemo(
    () => proofs.filter((p) => p.status === "submitted").reduce((sum, p) => sum + Number(p.amount), 0),
    [proofs],
  );
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const proofsBody = await proofsRes.json() as { proofs: PaymentProof[] };
  ...
  async function submitProof() {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE}/bookings/${bookingId}/payment-proofs`, {
        ...
        body: JSON.stringify({ amount, currency, method, proofUri, reference: reference.trim() || undefined, note: note.trim() || undefined }),
      });
  ...
  if (user?.role !== "client") {
    return (
      <View style={styles.centered}>
        <Feather name="shield" size={36} color={C.destructive} />
        <Text style={styles.centeredTitle}>هذه الشاشة مخصصة للعميل</Text>
        <TouchableOpacity onPress={() => router.back()}><Text style={styles.link}>العودة</Text></TouchableOpacity>
      </View>
    );
  }
    ...
    >
      <View style={styles.header}>
        ...
        <View style={styles.headerText}>
          ...
          <Text style={styles.subtitle}>ادفع من خلال مستشارك أولًا، أو سجّل دفعة خارجية عند الحاجة</Text>
        </View>
      </View>
      ...
      <View style={styles.primaryCard}>
        ...
        <View style={styles.primaryCopy}>
          <Text style={styles.primaryTitle}>الدفع عبر مستشارك هو الخيار الأساسي</Text>
          <Text style={styles.primaryText}>Visa وMastercard ووسائل الدفع المحلية المتاحة في بلدك تُعالج عبر المنصة عند تفعيل بوابة الدفع.</Text>
        </View>
        <TouchableOpacity style={styles.primaryButton} onPress={() => Alert.alert("الدفع الآمن", "سيتم فتح بوابة الدفع الآمنة من مستشارك عند تفعيل بوابة الدفع الإنتاجية. لم يتم تغيير Mock Payment في هذه المرحلة.") }>
          <Feather name="credit-card" size={16} color="#fff" />
          <Text style={styles.primaryButtonText}>الدفع عبر مستشارك</Text>
        </TouchableOpacity>
      </View>

      <View style={styles.secondaryCard}>
        <View style={styles.sectionHeader}>
          <View style={styles.sectionIcon}><Feather name="upload" size={15} color={C.gold} /></View>
          <View style={{ flex: 1 }}>
            <Text style={styles.sectionTitle}>الدفع خارج التطبيق</Text>
            <Text style={styles.sectionHint}>مسار بديل لمن لا يستطيع استخدام الدفع الإلكتروني</Text>
          </View>
        </View>
        ...
        <View style={styles.balanceRow}>
          <View><Text style={styles.balanceLabel}>إجمالي الخدمة</Text><Text style={styles.balanceValue}>{totalDue.toFixed(2)} {currency}</Text></View>
          <View><Text style={styles.balanceLabel}>بانتظار المراجعة</Text><Text style={styles.balanceValue}>{pendingTotal.toFixed(2)} {currency}</Text></View>
          <View><Text style={styles.balanceLabel}>المتبقي</Text><Text style={styles.balanceValue}>{remaining.toFixed(2)} {currency}</Text></View>
        </View>
        ...
        <View style={styles.methodRow}>
          {(Object.keys(METHOD_LABELS) as PaymentProof["method"][]).map((item) => (
            <TouchableOpacity key={item} onPress={() => setMethod(item)} style={[styles.methodButton, method === item && styles.methodButtonActive]}>
              <Text style={[styles.methodText, method === item && styles.methodTextActive]}>{METHOD_LABELS[item]}</Text>
            </TouchableOpacity>
          ))}
        </View>

        <TouchableOpacity style={styles.uploadButton} onPress={pickProof}>
          <Feather name={proofUri ? "check-circle" : "paperclip"} size={18} color={proofUri ? C.success : C.navy} />
          <Text style={styles.uploadText}>{proofUri ? "تم إرفاق إثبات الدفع" : "رفع إثبات الدفع"}</Text>
        </TouchableOpacity>
        ...
        <TouchableOpacity style={[styles.submitButton, submitting && { opacity: 0.6 }]} onPress={submitProof} disabled={submitting}>
          {submitting ? <ActivityIndicator color="#fff" size="small" /> : <><Feather name="send" size={17} color="#fff" /><Text style={styles.submitText}>إرسال إثبات الدفع</Text></>}
        </TouchableOpacity>
      </View>

      <View style={styles.historyCard}>
        <View style={styles.sectionHeader}>
          <View style={styles.sectionIcon}><Feather name="list" size={15} color={C.gold} /></View>
          <Text style={styles.sectionTitle}>سجل الدفعات</Text>
        </View>
        {loading ? <ActivityIndicator color={C.primary} /> : proofs.length === 0 ? (
          <Text style={styles.emptyText}>لا توجد دفعات خارجية مسجلة.</Text>
        ) : proofs.map((proof) => {
          const meta = STATUS_META[proof.status];
          return (
            <View key={proof.id} style={styles.proofRow}>
              <View style={{ flex: 1 }}>
                <Text style={styles.proofAmount}>{Number(proof.amount).toFixed(2)} {proof.currency}</Text>
                <Text style={styles.proofMethod}>{METHOD_LABELS[proof.method]} • {new Date(proof.submittedAt).toLocaleDateString("ar-SA")}</Text>
                {proof.rejectionReason && <Text style={styles.rejection}>{proof.rejectionReason}</Text>}
              </View>
      ...
      <View style={styles.notice}>
        ...
        <Text style={styles.noticeText}>لأسباب الأمان، مستشارك لا تعتبر إيصال العميل دليلًا نهائيًا على استلام المال. التأكيد يتم من المحامي، أما الدفع الإلكتروني فيعتمد على تأكيد بوابة الدفع.</Text>
      </View>
    </ScrollView>
  );
}
