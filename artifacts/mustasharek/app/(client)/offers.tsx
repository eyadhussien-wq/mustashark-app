...
const offers: Offer[] = [
  ...
  { id: "rep-1", kind: "representation", status: "draft", amount: 4500, title: "توكيل وتمثيل — نزاع مدني", note: "عرض أتعاب مقسم حسب مراحل القضية." },
];
...
export default function ClientOffers() {
  ...
  return <ScrollView style={styles.container} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
    ...
    <View style={styles.legend}>
      <Feather name="shield" size={15} color={C.gold} /><Text style={styles.legendText}>لا يبدأ تنفيذ الخدمة من هذه الشاشة؛ القبول والدفع الحقيقيان سيُربطان بالـBackend لاحقًا.</Text>
    </View>
    {offers.map((offer) => <View key={offer.id}>
      <UnifiedServiceOfferCard kind={offer.kind} status={offer.status} amount={offer.amount} title={offer.title} onPress={() => router.push({ pathname: "/(client)/offer", params: { id: offer.id, kind: offer.kind } })} />
