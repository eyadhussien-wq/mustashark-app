          ...
          <>
            <View style={styles.brandHeader}>
              <View style={styles.brandCopy}>
                ...
                <Text style={styles.greetingSub}>ماذا تحتاج اليوم لاستشارتك القانونية؟</Text>
              </View>
            ...
            {nextConsultation ? (
              ...
              >
                <View style={styles.nextTop}>
                  <View style={styles.nextLabelRow}>
                    ...
                    <Text style={styles.nextLabel}>نشاطك الحالي</Text>
                  </View>
                ...
                <View style={styles.nextMetaRow}>
                  ...
                  <Meta
                    icon={nextConsultation.type === "video" ? "video" : nextConsultation.type === "phone" ? "phone" : "message-square"}
                    text={nextConsultation.type === "video" ? "فيديو" : nextConsultation.type === "phone" ? "هاتف" : "محادثة"}
                  />
                </View>
              ...
              <View style={styles.noBookingCard}>
                <View style={styles.noBookingIcon}>
                ...
                <View style={styles.noBookingCopy}>
                  ...
                  <Text style={styles.noBookingText}>ابدأ من هنا بالعثور على المحامي المناسب.</Text>
                </View>
              </View>
            )}
            ...
            <View style={styles.searchSection}>
              <View style={styles.sectionHeader}>
                <View style={styles.sectionCopy}>
                  <Text style={styles.sectionKicker}>ابدأ من هنا</Text>
                  <Text style={styles.sectionTitle}>ابحث عن محامٍ</Text>
                  <Text style={styles.sectionSubtitle}>ابحث بالاسم أو التخصص ثم اختر المحامي المناسب</Text>
                </View>
                <View style={styles.searchIconBadge}>
                  <Feather name="search" size={19} color={C.navy} />
                </View>
              </View>
              ...
              <View style={styles.searchBar}>
                ...
                <TextInput
                  ...
                  accessibilityLabel="البحث عن محامٍ"
                />
                {!!search && (
              ...
              <View style={styles.filtersBlock}>
                <FlatList
                  ...
                  renderItem={({ item }) => (
                    <TouchableOpacity
                      style={[styles.chip, country === item && styles.chipActive]}
            ...
            <View style={styles.discoveryHeader}>
              <View style={styles.discoveryCopy}>
                <Text style={styles.discoveryTitle}>{hasActiveSearch ? "نتائج البحث" : "محامون مقترحون"}</Text>
                <Text style={styles.discoverySubtitle}>
                  {hasActiveSearch ? `${filtered.length} محامٍ مطابق` : "خيارات مناسبة لتبدأ منها"}
                </Text>
              </View>
              {!hasActiveSearch && filtered.length > RECOMMENDED_LIMIT && (
                <TouchableOpacity
                  style={styles.showAllButton}
                  onPress={() => setShowAllLawyers((value) => !value)}
                >
                  <Text style={styles.showAllButtonText}>{showAllLawyers ? "عرض أقل" : "عرض الكل"}</Text>
                  <Feather name={showAllLawyers ? "chevron-up" : "chevron-left"} size={15} color={C.primary} />
                </TouchableOpacity>
              )}
            </View>
          </>
          ...
          <View style={styles.empty}>
            ...
            {hasActiveSearch && (
              <TouchableOpacity style={styles.resetButton} onPress={resetDiscovery}>
                <Text style={styles.resetButtonText}>مسح البحث والفلاتر</Text>
              </TouchableOpacity>
            )}
          </View>
        }
...
function Meta({ icon, text }: { icon: string; text: string }) {
  return (
    <View style={styles.meta}>
      <Feather name={icon as any} size={13} color="rgba(255,255,255,0.75)" />
...
const styles = StyleSheet.create({
  ...
  nextCard: { backgroundColor: C.navy, borderRadius: 18, padding: 18, marginBottom: 18, borderWidth: 1, borderColor: "rgba(201,160,53,0.42)" },
  ...
  noBookingCard: { flexDirection: "row", alignItems: "center", backgroundColor: C.card, borderWidth: 1, borderColor: C.border, borderRadius: 16, padding: 15, marginBottom: 18, gap: 12 },
  ...
  searchSection: { backgroundColor: C.card, borderRadius: 18, borderWidth: 1, borderColor: C.border, padding: 16, marginBottom: 18 },
  sectionHeader: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginBottom: 13 },
  sectionCopy: { flex: 1, alignItems: "flex-end", marginLeft: 12 },
  sectionKicker: { fontSize: 10, color: C.gold, fontFamily: "Inter_700Bold", textAlign: "right", marginBottom: 2 },
  sectionTitle: { fontSize: 18, fontFamily: "Inter_700Bold", color: C.foreground, textAlign: "right" },
  sectionSubtitle: { fontSize: 11, color: C.mutedForeground, fontFamily: "Inter_400Regular", textAlign: "right", marginTop: 3 },
  searchIconBadge: { width: 42, height: 42, borderRadius: 13, backgroundColor: "#EEF2F8", alignItems: "center", justifyContent: "center" },
  searchBar: { flexDirection: "row", alignItems: "center", gap: 10, backgroundColor: C.background, borderRadius: 13, borderWidth: 1, borderColor: C.border, paddingHorizontal: 14, paddingVertical: 12, marginBottom: 11 },
  ...
  discoveryHeader: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginBottom: 12 },
  discoveryCopy: { flex: 1, alignItems: "flex-end" },
  discoveryTitle: { fontSize: 17, fontFamily: "Inter_700Bold", color: C.foreground, textAlign: "right" },
  discoverySubtitle: { fontSize: 11, color: C.mutedForeground, fontFamily: "Inter_400Regular", textAlign: "right", marginTop: 2 },
  showAllButton: { flexDirection: "row", alignItems: "center", gap: 4, paddingHorizontal: 9, paddingVertical: 7, borderRadius: 10, backgroundColor: "#EEF2F8" },
  showAllButtonText: { fontSize: 11, color: C.primary, fontFamily: "Inter_600SemiBold" },
