  ...
  for (let i = 0; i < 30; i++) {
    ...
    if (workingDays.includes(dayOfWeek)) {
      ...
      days.push({
        ...
        dayOfWeek,
        dayNum,
        monthLabel: lang === "ar" ? monthLabelsAR[month] : monthLabelsEN[month],
        weekdayLabel: lang === "ar" ? dayLabelsAR[dayOfWeek] : dayLabelsEN[dayOfWeek],
      });
    }
  }
  return days;
...
export default function LawyerDetail() {
  ...
  const slots = useMemo(
    () => (selectedDate ? getAvailableSlots(lawyer!.id, selectedDate) : []),
    [selectedDate, lawyer, getAvailableSlots]
  );

  if (!lawyer) {
    return (
      <View style={styles.notFound}>
        <Text style={styles.notFoundText}>{t("noLawyersFound")}</Text>
  ...
  function handleProceed() {
    setError("");
    if (!subject.trim()) { setError(t("error")); return; }
    if (!description.trim()) { setError(t("error")); return; }
    if (!selectedDate) { setError(t("error")); return; }
    if (!selectedTime) { setError(t("error")); return; }
    if (!user) return;
    ...
    >
      ...
      >
        ...
        <View style={styles.profileCard}>
          <View style={[styles.profileHeader, { flexDirection: rowDir }]}>
        ...
        <View style={styles.section}>
          <Text style={[styles.sectionTitle, { textAlign: textAlign }]}>{t("availability")}</Text>
              ...
              {calendarDays.map((day) => {
                ...
                return (
                  <TouchableOpacity
                    ...
                    activeOpacity={0.8}
                  >
                    <Text style={[styles.dayMonth, active && styles.dayCardTextActive]}>{day.monthLabel}</Text>
                    <Text style={[styles.dayNum, active && styles.dayCardNumActive]}>{day.dayNum}</Text>
                    <Text style={[styles.dayLabel, active && styles.dayCardTextActive]}>{day.weekdayLabel}</Text>
                  </TouchableOpacity>
                );
              })}
            </View>
        ...
        {selectedDate && (
          <View style={styles.section}>
            <Text style={[styles.sectionTitle, { textAlign: textAlign }]}>
              {t("startHour")} — {t("endHour")} ({lawyer.availability?.slotDuration ?? 60} {t("minutes")})
            </Text>
            {slots.length === 0 ? (
              <Text style={[styles.emptyText, { textAlign: textAlign }]}>{t("noLawyersFound")}</Text>
            ) : (
                ...
                {slots.map((slot) => (
                  <TouchableOpacity
                    ...
                    style={[
                      ...
                      selectedTime === slot.time && styles.slotChipActive,
                    ]}
                  ...
                  >
                    ...
                    {!slot.available && (
                      <Text style={styles.slotBookedLabel}>{t("notAvailable")}</Text>
                    )}
                  </TouchableOpacity>
                ))}
              </View>
              ...
              return (
                ...
                >
                  <View style={[styles.typeIcon, active && styles.typeIconActive]}>
                    <Feather name={type.icon as any} size={18} color={active ? "#fff" : C.primary} />
                  </View>
        ...
        <View style={styles.field}>
          <Text style={[styles.label, { textAlign: textAlign }]}>{t("subject")}</Text>
          <TextInput
            style={[styles.input, { textAlign: textAlign }]}
            value={subject}
            onChangeText={setSubject}
            placeholder={lang === "ar" ? "موضوع الاستشارة" : "Consultation subject"}
        ...
        {/* Description */}
        <View style={styles.field}>
          <Text style={[styles.label, { textAlign: textAlign }]}>{t("description")}</Text>
          <TextInput
            style={[styles.input, styles.textarea, { textAlign: textAlign }]}
            value={description}
            onChangeText={setDescription}
            placeholder={lang === "ar" ? "وصف المشكلة باختصار" : "Brief problem description"}
            placeholderTextColor={C.mutedForeground}
            multiline
            numberOfLines={4}
        ...
        {/* Attachments */}
        <View style={styles.field}>
          <View style={[styles.labelRow, { flexDirection: rowDir }]}>
            <Text style={[styles.label, { textAlign: textAlign }]}>{t("attachments")}</Text>
            {attachments.length < 5 && (
              <TouchableOpacity onPress={pickAttachment} style={styles.attachBtn}>
                <Feather name="paperclip" size={14} color={C.primary} />
                <Text style={styles.attachText}>{t("add")}</Text>
              </TouchableOpacity>
            )}
          </View>
          {attachments.map((a, i) => (
            <View key={i} style={[styles.attachRow, { flexDirection: rowDir }]}>
              ...
              <Feather name="file" size={14} color={C.primary} />
            </View>
          ))}
        </View>
        ...
        <TouchableOpacity style={styles.proceedBtn} onPress={handleProceed} activeOpacity={0.85}>
          <Text style={styles.proceedText}>{t("submitRequest")}</Text>
          <Feather name={lang === "ar" ? "arrow-left" : "arrow-right"} size={16} color="#fff" />
        </TouchableOpacity>
      </ScrollView>
    </KeyboardAvoidingView>
  );
...
const styles = StyleSheet.create({
  ...
  notFoundText: { fontSize: 18, color: C.foreground, fontFamily: "Inter_600SemiBold" },
  backLink: { fontSize: 15, color: C.primary, fontFamily: "Inter_500Medium" },

  profileCard: {
    backgroundColor: C.card, borderRadius: colors.radius,
    borderWidth: 1, borderColor: C.border, padding: 18, marginBottom: 20, gap: 14,
  },
  ...
  lawyerName: { fontSize: 18, fontFamily: "Inter_700Bold", color: C.foreground },
  lawyerSpec: { fontSize: 13, color: C.primary, fontFamily: "Inter_500Medium" },
  ...
  daysRow: { flexDirection: "row", gap: 8, paddingRight: 4 },
  dayCard: {
  ...
  dayMonth: { fontSize: 10, color: C.mutedForeground, fontFamily: "Inter_400Regular" },
  dayNum: { fontSize: 18, fontFamily: "Inter_700Bold", color: C.foreground },
  dayLabel: { fontSize: 10, color: C.mutedForeground, fontFamily: "Inter_400Regular" },
  dayCardTextActive: { color: "rgba(255,255,255,0.65)", fontFamily: "Inter_500Medium" },
  dayCardNumActive: { color: "#fff", fontFamily: "Inter_700Bold" },
  ...
  slotChipActive: { backgroundColor: C.navy, borderColor: C.navy },
  slotChipDisabled: { backgroundColor: "#F9FAFB", borderColor: C.border, opacity: 0.5 },
  slotText: { fontSize: 14, fontFamily: "Inter_600SemiBold", color: C.foreground },
  slotTextActive: { color: "#fff" },
  slotTextDisabled: { color: C.mutedForeground, fontFamily: "Inter_400Regular" },
  slotBookedLabel: { fontSize: 9, color: C.destructive, fontFamily: "Inter_400Regular", marginTop: 2 },
  ...
  typeLabel: { fontSize: 12, fontFamily: "Inter_500Medium", color: C.foreground, textAlign: "center" },
  typeLabelActive: { color: "#fff", fontFamily: "Inter_600SemiBold" },

  field: { marginBottom: 18 },
  label: { fontSize: 13, fontFamily: "Inter_600SemiBold", color: C.foreground, marginBottom: 8 },
  ...
  attachText: { fontSize: 13, color: C.primary, fontFamily: "Inter_500Medium" },
  attachRow: { flexDirection: "row", alignItems: "center", gap: 8, paddingVertical: 6 },
  attachName: { flex: 1, fontSize: 13, color: C.foreground, fontFamily: "Inter_400Regular" },
