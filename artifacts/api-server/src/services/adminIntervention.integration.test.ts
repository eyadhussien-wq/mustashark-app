...
if (!testDatabaseUrl) {
  test("S02-08 Admin Intervention: isolated test database is required", () => {
    assert.fail(
      "ADMIN_INTERVENTION_TEST_DATABASE_URL is required; refusing to run S02-08 tests without an isolated Test DB",
    );
  });
}

if (testDatabaseUrl && /(prod|production|live)/i.test(testDatabaseUrl)) {
  test("S02-08 Admin Intervention: production database URL is forbidden", () => {
    assert.fail("ADMIN_INTERVENTION_TEST_DATABASE_URL appears to reference production; refusing to run");
  });
}
...
const createFixture = async () => {
  ...
  assert.equal(admin.role, "admin", `${adminEmail} must have admin role`);
  assert.equal(admin.accountStatus, "active", `${adminEmail} must be active`);
  assert.equal(admin.deletedAt, null, `${adminEmail} must not be deleted`);
  ...
  await db.insert(representationQuotesTable).values({
    ...
    clientId,
    lawyerId,
    title: "S02-08 Test Quote",
    description: "Isolated admin intervention fixture",
    totalAmount: "100.00",
    currency: "JOD",
    status: "active",
    fundingMode: "full",
  ...
  return { adminId: admin.id, caseId };
};
...
test("S02-08: admin intervention atomically changes case and writes immutable audit", async () => {
  ...
  const [audit] = await db
    ...
    .where(and(eq(adminAuditLogsTable.adminId, adminId), eq(adminAuditLogsTable.entityId, caseId)))
    .limit(1);
...
test("S02-08: failure after case update rolls back both case mutation and audit insert", async () => {
  ...
  // Test-only fault injection: deleting the admin after the case UPDATE makes the
  // subsequent audit INSERT fail its FK. The surrounding transaction must roll back.
  await pool.query(`
    CREATE OR REPLACE FUNCTION s02_08_force_admin_audit_fk_failure()
    RETURNS trigger LANGUAGE plpgsql AS $$
    BEGIN
      DELETE FROM users WHERE id = '${adminId.replace(/'/g, "''")}';
      RETURN NEW;
    END;
    $$;
  `);
  await pool.query(`
    CREATE TRIGGER s02_08_force_admin_audit_fk_failure
    AFTER UPDATE OF status ON cases
    FOR EACH ROW
    WHEN (NEW.id = '${caseId.replace(/'/g, "''")}')
    EXECUTE FUNCTION s02_08_force_admin_audit_fk_failure();
  `);

  const auditBefore = await db
    .select({ id: adminAuditLogsTable.id })
    .from(adminAuditLogsTable)
    .where(eq(adminAuditLogsTable.entityId, caseId));
  ...
  await assert.rejects(
    ...
    /violates foreign key constraint|ADMIN_INTERVENTION_FORBIDDEN|insert or update/i,
  );

  await pool.query("DROP TRIGGER s02_08_force_admin_audit_fk_failure ON cases");
  await pool.query("DROP FUNCTION s02_08_force_admin_audit_fk_failure()");
...
test("S02-08: admin audit logs reject UPDATE and DELETE with PostgreSQL 42501", async () => {
  ...
  await assert.rejects(
    () => pool.query("UPDATE admin_audit_logs SET description = 'tampered' WHERE id = $1", [audit.id]),
    (error: unknown) => (error as { code?: string }).code === "42501",
  );

  await assert.rejects(
    () => pool.query("DELETE FROM admin_audit_logs WHERE id = $1", [audit.id]),
    (error: unknown) => (error as { code?: string }).code === "42501",
  );
...
after(async () => {
  // Audit rows are intentionally retained in the isolated DB because production
  // semantics forbid deleting immutable audit history. The CI container is ephemeral.
