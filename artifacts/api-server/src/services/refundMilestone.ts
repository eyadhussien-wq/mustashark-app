...
/**
 * Refunds a funded milestone before release. The amount/currency are read
 * from the database; the client cannot choose financial values. All ledger,
 * escrow and milestone mutations are atomic and idempotent.
 */
export async function refundMilestone(
  req: Request,
  milestoneId: string,
  clientId: string,
): Promise<RefundMilestoneResult> {
  return db.transaction(async (tx) => {
    ...
    if (["released", "cancelled", "disputed"].includes(row.milestone.status)) {
      return { error: "milestone_not_refundable" };
    }
    ...
    const [updatedEscrow] = await tx
      ...
      .set({
        refundedAmount: sql`${escrowAccountsTable.refundedAmount} + ${amount}`,
        updatedAt: now,
      })
      .where(and(
