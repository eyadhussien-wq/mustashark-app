...
export async function createRepresentationQuoteRequest(req: Request, res: Response) {
  ...
  try {
    const result = await db.transaction(async (tx) => {
      if (parsed.data.lawyerId) {
        const [lawyer] = await tx.select({ id: usersTable.id }).from(usersTable).where(and(
          eq(usersTable.id, parsed.data.lawyerId), eq(usersTable.role, "lawyer"), eq(usersTable.accountStatus, "active"),
        )).limit(1);
        if (!lawyer) return { validationError: "lawyer_not_found_or_unavailable" as const };
      }
    ...
    if ("validationError" in result) return res.status(404).json({ ok: false, error: result.validationError });
...
export async function listLawyerRepresentationRequests(req: Request, res: Response) {
