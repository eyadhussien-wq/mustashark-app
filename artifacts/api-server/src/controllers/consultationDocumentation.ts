...
export async function listConsultationArchive(req: Request, res: Response) {
  ...
  const rows = await db
    ...
    .from(bookingsTable)
    .where(and(isNotNull(bookingsTable.archivedAt), inArray(bookingsTable.status, [...terminalStatuses]), user.role === "admin" ? undefined : user.role === "lawyer" ? eq(bookingsTable.lawyerId, user.id) : eq(bookingsTable.clientId, user.id)))
...
export async function archiveConsultation(req: Request, res: Response) {
  ...
  const result = await db.transaction(async (tx) => {
    ...
    const [updated] = await tx.update(bookingsTable).set({ archivedAt, archivedBy: req.authUser!.id, updatedAt: archivedAt }).where(and(eq(bookingsTable.id, bookingId), isNull(bookingsTable.archivedAt))).returning();
    if (!updated) {
      const [alreadyArchived] = await tx.select().from(bookingsTable).where(eq(bookingsTable.id, bookingId)).limit(1);
      return alreadyArchived ? { kind: "already_archived" as const, booking: alreadyArchived } : { kind: "not_found" as const };
    }

    await tx.insert(consultationEventsTable).values({ id: randomUUID(), bookingId, eventType: "consultation.archived", actorId: req.authUser!.id, metadata: { reason: parsed.data.reason ?? null } });
    if (req.authUser!.role === "admin") {
      await tx.insert(adminAuditLogsTable).values({ id: randomUUID(), adminId: req.authUser!.id, action: "consultation.archived", entityType: "booking", entityId: bookingId, description: parsed.data.reason ?? "Consultation archived", beforeData: { archivedAt: null, status: booking.status }, afterData: { archivedAt, status: updated.status } });
    }
...
async function loadConsultationForActor(req: Request) {
  const bookingId = String(req.params.id ?? "");
  const [row] = await db.select({ booking: bookingsTable }).from(bookingsTable).where(eq(bookingsTable.id, bookingId)).limit(1);
...
export async function getConsultationPrintData(req: Request, res: Response) {
  const bookingId = String(req.params.id ?? "");
  const [row] = await db.select({ booking: bookingsTable, client: usersTable }).from(bookingsTable).leftJoin(usersTable, eq(bookingsTable.clientId, usersTable.id)).where(eq(bookingsTable.id, bookingId)).limit(1);
