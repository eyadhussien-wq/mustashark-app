 ...
 * Financial/administrative state is intentionally limited to fields that
 * actually exist in the canonical booking schema.
 */
function toSafeBooking(
  booking: typeof bookingsTable.$inferSelect,
  names: {
    clientName?: string;
    lawyerName?: string;
    lawyerSpecialization?: string;
    lawyerCountry?: string | null;
  },
) {
  return {
    id: booking.id,
...
async function enrichBooking(booking: typeof bookingsTable.$inferSelect) {
  ...
  const [lawyer] = booking.lawyerId
    ? await db
        ...
        .from(usersTable)
        .where(eq(usersTable.id, booking.lawyerId))
        .limit(1)
    : [];
...
export const getBookingByIdSafe = async (req: Request, res: Response) => {
  try {
    ...
    if (!bookingId) {
      return res.status(400).json({ ok: false, error: "booking_id_is_required" });
    }
    const authUser = req.authUser!;
