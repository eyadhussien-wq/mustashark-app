...
const APPLE_CLIENT_ID =
  process.env.APPLE_CLIENT_ID ?? "com.mustasharek.app";
...
function lawyerVerificationPendingResponse() {
  return {
    ...
    message:
      "تم استلام طلب تسجيل المحامي. لا يمكن استخدام بوابة المحامين قبل التحقق المهني واعتماد الإدارة.",
  };
}
  ...
  for (
    ...
    if (dbError.code === "23505") {
      ...
      if (
        ...
        return dbError.constraint_name;
      }
    }
...
const OAUTH_CREATE_MAX_ATTEMPTS = 3;

const OAUTH_CREATE_RETRY_DELAYS_MS =
  [50, 100] as const;
...
async function getRegistrationTerms(
  tx: Parameters<
    Parameters<typeof db.transaction>[0]
  >[0],
  termsVersionId: string | undefined,
  termsContentHash: string | undefined,
) {
  if (
    !termsVersionId ||
    !termsContentHash
  ...
  const rows = await tx
    ...
    .where(
      and(
        eq(
          termsVersionsTable.status,
          "published",
        ),
        eq(
          termsVersionsTable.mandatory,
          true,
        ),
        lte(
          termsVersionsTable.effectiveAt,
          new Date(),
        ),
      ),
    )
    .orderBy(
      desc(termsVersionsTable.version),
    )
    .limit(1);
  ...
  if (
    current.id !== termsVersionId ||
    current.contentHash.toLowerCase() !==
      termsContentHash.toLowerCase()
  ) {
    throw new Error(
...
function termsRegistrationError(
  res: Response,
  error: unknown,
) {
  const message =
...
export async function socialAuth(
  ...
  const parsed =
    socialSchema.safeParse(req.body);
  ...
  try {
    ...
    if (!dbUser) {
      if (!providerUser.email) {
        return res.status(400).json({
          ...
          message:
            "البريد الإلكتروني الموثق من مزود الهوية مطلوب لإنشاء حساب جديد.",
        });
      }

      const newId =
        `${provider}-${providerUser.id.substring(
          0,
          12,
        ...
        try {
          inserted =
            await db.transaction(
              async (tx) => {
                await tx.execute(
                  sql`SELECT pg_advisory_xact_lock(hashtextextended(${`${provider}:${providerUser.id}`}, 0))`,
                );

                const existingProviderRows =
                  await tx
                    .select()
                    .from(usersTable)
                    .where(
                ...
                if (
                  existingProviderRows[0]
                ) {
                  return existingProviderRows[0];
                }

                const current =
                  await getRegistrationTerms(
                    tx,
                    termsVersionId,
                    termsContentHash,
                  );
          ...
          if (
            attempt <
            OAUTH_CREATE_MAX_ATTEMPTS - 1
          ) {
            await sleep(
              OAUTH_CREATE_RETRY_DELAYS_MS[
                attempt
              ],
            );
          }
    ...
    if (
      ...
      if (
        ...
        return res.status(403).json({
          ok: false,
          error:
            "account_reactivation_requires_verified_auth",
          message:
            "لا يمكن إعادة تفعيل الحساب المحذوف عبر تسجيل الدخول الاجتماعي دون مسار تحقق موثق.",
        });
      }
      ...
      return res.status(403).json({
        ...
        message:
          "عذراً، انتهت مدة استعادة الحساب (30 يوماً). تم حذف الحساب نهائياً.",
      });
    }
    ...
    if (
      ...
      return res.status(403).json({
        ...
        message:
          "عذراً، تم إيقاف هذا الحساب. يرجى التواصل مع الدعم.",
      });
    }
    ...
    if (
      ...
      return res
        .status(403)
        .json(
          lawyerVerificationPendingResponse(),
        );
    }
    ...
    if (
      ...
      return res.status(403).json({
        ...
        error:
          "account_not_active",
        message:
          "عذراً، لا يمكن تسجيل الدخول بهذا الحساب حالياً.",
      });
    }
    ...
    if (
      ...
      return res.status(403).json({
        ...
        message:
          "لا يمكن ربط حساب اجتماعي بحساب محلي أثناء تسجيل الدخول. استخدم مسار الربط الموثق من داخل الحساب.",
      });
    }
    ...
    return res.json({
      ...
      roleUi:
        ROLE_UI[
          dbUser.role as keyof typeof ROLE_UI
        ],
      user: {
        id: dbUser.id,
        name: dbUser.name,
        ...
        authProvider:
          dbUser.authProvider,
        deletionRejectionNote:
          dbUser.deletionRejectionNote ??
          null,
        createdAt:
          dbUser.createdAt,
      },
...
export async function localAuth(
  ...
  const DUMMY_HASH =
    "$2b$10$dummyhashfortimingsafetyXXXXXXXXXXXXXXXXX";
  ...
  try {
    ...
    if (existing) {
      if (existing.passwordHash) {
        ...
        req.log.warn(
          { userId: existing.id },
          "local-auth: rejected password-link attempt for social-only account",
        );
        ...
        return res.status(403).json({
          ...
          error:
            "social_account_only",
          message:
            "هذا الحساب مرتبط بتسجيل دخول اجتماعي (Google/Apple). يرجى استخدام نفس طريقة التسجيل.",
        });
      }
      ...
      return res.json({
        ...
        roleUi:
          ROLE_UI[
            refreshed.role as keyof typeof ROLE_UI
          ],
        user: {
          id: refreshed.id,
          name: refreshed.name,
          ...
          phoneCountry:
            refreshed.phoneCountry,
          country:
            refreshed.country,
          nationality:
            refreshed.nationality,
          specialization:
            refreshed.specialization,
          bio: refreshed.bio,
          hourlyRate:
            refreshed.hourlyRate
              ? parseFloat(
                  refreshed.hourlyRate,
                )
              : null,
    ...
    if (!isSupportedPhone(phone)) {
      return res.status(400).json({
        ...
        error: "invalid_phone",
        message:
          "رقم الهاتف غير صالح. استخدم رقمًا قطريًا يبدأ +974 أو أردنيًا يبدأ +962 وبالطول الصحيح.",
      });
    }
    ...
    const isLawyerRegistration =
      role === "lawyer";

    const accountStatus =
      isLawyerRegistration
        ? "pending"
        : "active";
    ...
    if (isLawyerRegistration) {
      return res.status(202).json({
        ...
        verificationRequired:
          true,
        message:
          "تم إنشاء طلب تسجيل المحامي. لا يمكن الدخول إلى بوابة المحامين حتى تتحقق الإدارة من الصفة المهنية وتعتمد الحساب.",
        user: {
          id: created.id,
          name: created.name,
          email: created.email,
