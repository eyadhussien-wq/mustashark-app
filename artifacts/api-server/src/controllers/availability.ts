...
const availabilitySchema = z.object({
  schedulingTimezone: z.string().min(1).max(128),
...
function wallClockToUtc(date: string, time: string, timezone: string) {
  const normalizedDate = normalizeDate(date);
  if (!normalizedDate || !isSupportedTimezone(timezone)) return null;
...
export const getLawyerAvailability = async (req: Request, res: Response) => {
  try {
    const lawyerId = req.params.lawyerId ?? req.authUser?.id;
    if (!lawyerId) return res.status(401).json({ ok: false, error: "unauthorized" });
    const [lawyer] = await db.select({ id: usersTable.id, role: usersTable.role, accountStatus: usersTable.accountStatus, schedulingTimezone: usersTable.schedulingTimezone })
      .from(usersTable).where(eq(usersTable.id, String(lawyerId))).limit(1);
    ...
    const rows = await db.select().from(lawyerAvailabilityTable)
      .where(and(eq(lawyerAvailabilityTable.lawyerId, String(lawyerId)), eq(lawyerAvailabilityTable.active, true)))
      .orderBy(asc(lawyerAvailabilityTable.dayOfWeek), asc(lawyerAvailabilityTable.startTime));
    return res.json({ ok: true, schedulingTimezone: lawyer.schedulingTimezone, availability: rows });
  } catch (error) {
...
export const updateMyAvailability = async (req: Request, res: Response) => {
  try {
    ...
    if (!parsed.success || !isSupportedTimezone(parsed.data?.schedulingTimezone ?? "")) return res.status(400).json({ ok: false, error: "invalid_availability_timezone" });
    ...
    for (const slot of parsed.data.slots) {
      ...
      if (end - start < slot.slotDurationMinutes) return res.status(400).json({ ok: false, error: "availability_window_shorter_than_slot_duration" });
    ...
    for (const day of byDay.values()) {
      ...
      for (let index = 1; index < day.length; index += 1) if (day[index].start < day[index - 1].end) return res.status(400).json({ ok: false, error: "availability_slots_overlap" });
    }
    const rows = await db.transaction(async (tx) => {
      await tx.update(usersTable).set({ schedulingTimezone: parsed.data.schedulingTimezone, updatedAt: new Date() }).where(eq(usersTable.id, lawyerId));
    ...
    return res.json({ ok: true, schedulingTimezone: parsed.data.schedulingTimezone, availability: rows });
  } catch (error) {
...
export const getAvailableSlots = async (req: Request, res: Response) => {
  try {
    ...
    const [lawyer] = await db.select({ id: usersTable.id, role: usersTable.role, accountStatus: usersTable.accountStatus, schedulingTimezone: usersTable.schedulingTimezone }).from(usersTable).where(eq(usersTable.id, lawyerId)).limit(1);
    ...
    if (!lawyer.schedulingTimezone || !isSupportedTimezone(lawyer.schedulingTimezone)) return res.status(409).json({ ok: false, error: "lawyer_scheduling_timezone_not_configured" });
    const dayOfWeek = new Date(`${date}T12:00:00Z`).getUTCDay();
    const availability = await db.select().from(lawyerAvailabilityTable).where(and(eq(lawyerAvailabilityTable.lawyerId, lawyerId), eq(lawyerAvailabilityTable.dayOfWeek, dayOfWeek), eq(lawyerAvailabilityTable.active, true))).orderBy(asc(lawyerAvailabilityTable.startTime));
    const blocks = await db.select({ block: bookingTimeBlocksTable }).from(bookingTimeBlocksTable).innerJoin(bookingsTable, eq(bookingTimeBlocksTable.bookingId, bookingsTable.id)).where(and(eq(bookingTimeBlocksTable.lawyerId, lawyerId), eq(bookingTimeBlocksTable.scheduledDate, date), notInArray(bookingsTable.status, ["rejected", "cancelled_by_lawyer", "cancelled_by_client", "refunded_absent"]))).orderBy(asc(bookingTimeBlocksTable.startTime));
    ...
    for (const window of availability) {
      ...
      for (let cursor = start; cursor + duration <= end; cursor += duration) {
        ...
        if (!occupied.some((block) => slotStart < block.end && slotEnd > block.start)) {
          const startTime = formatTime(slotStart);
          const endTime = formatTime(slotEnd);
          const startAtUtc = wallClockToUtc(date, startTime, lawyer.schedulingTimezone);
          const endAtUtc = wallClockToUtc(date, endTime, lawyer.schedulingTimezone);
          if (startAtUtc && endAtUtc) slots.push({ startTime, endTime, startAtUtc, endAtUtc });
        }
      }
    }
    return res.json({ ok: true, date, timezone: lawyer.schedulingTimezone, slots });
  } catch (error) {
