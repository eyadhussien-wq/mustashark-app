...
export const getAvailableSlots = async (req: Request, res: Response) => {
  try {
    ...
    const [lawyer] = await db.select({ id: usersTable.id, role: usersTable.role, accountStatus: usersTable.accountStatus }).from(usersTable).where(eq(usersTable.id, lawyerId)).limit(1);
    ...
    for (const window of availability) {
      const start = minutes(window.startTime); const end = minutes(window.endTime); const duration = window.slotDurationMinutes;
      for (let cursor = start; cursor + duration <= end; cursor += duration) {
        const slotStart = cursor; const slotEnd = cursor + duration;
        if (!occupied.some((block) => slotStart < block.end && slotEnd > block.start)) {
          const startTime = formatTime(slotStart); const endTime = formatTime(slotEnd);
          const startAtUtc = wallClockToUtc(date, startTime); const endAtUtc = wallClockToUtc(date, endTime);
