...
const rejectPaymentProofSchema = z.object({ reason: z.string().trim().min(1).max(1000) });

const CLOSED_BOOKING_STATUSES = ["rejected", "completed", "cancelled_by_client", "cancelled_by_lawyer", "no_show_client", "no_show_lawyer", "refunded_absent"] as const;
...
async function resolveBookingCurrency(tx: DbLike, booking: { lawyerId: string | null; officeId: string | null }) {
  if (booking.officeId) {
    ...
    const currency = currencyForCountry(office?.country);
  ...
  if (booking.lawyerId) {
    ...
    const currency = currencyForCountry(lawyer?.country);
    if (currency) return currency;
  }
  return null;
}
...
export const submitPaymentProof = async (req: Request, res: Response) => {
  try {
    ...
    const result = await db.transaction(async (tx) => {
      ...
      const currency = await resolveBookingCurrency(tx, booking);
      if (!currency) return { kind: "currency_not_configured" as const };
      ...
      const [proof] = await tx.insert(paymentProofsTable).values({
        id: crypto.randomUUID(), bookingId, clientId: authUser.id, amount: requested.toFixed(2), currency,
...
export const confirmPaymentProof = async (req: Request, res: Response) => {
  try {
    ...
    const result = await db.transaction(async (tx) => {
      ...
      const currency = await resolveBookingCurrency(tx, booking);
      if (!currency) return { kind: "currency_not_configured" as const };
      if (proof.currency !== currency) return { kind: "currency_mismatch" as const };
