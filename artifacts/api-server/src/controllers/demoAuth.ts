...
const demoSchema = z.object({
  email: z.enum(["client@mustashark.com", "lawyer@mustashark.com"]),
  password: z.literal("test1234"),
...
export async function demoAuth(req: Request, res: Response) {
  ...
  if (!parsed.success || (parsed.data.role === "client" && parsed.data.email !== "client@mustashark.com") || (parsed.data.role === "lawyer" && parsed.data.email !== "lawyer@mustashark.com")) {
    return res.status(401).json({ ok: false, error: "invalid_demo_credentials" });
  }
  ...
  try {
    ...
    if (!existing) {
      await db.insert(usersTable).values({
        ...
        ...(parsed.data.role === "lawyer" ? { specialization: demo.specialization, bio: demo.bio, hourlyRate: demo.hourlyRate } : {}),
        createdAt: new Date(),
        updatedAt: new Date(),
      });
      ...
      return res.status(403).json({ ok: false, error: "role_mismatch" });
    }
    ...
    req.log.error(err, "demoAuth failed");
