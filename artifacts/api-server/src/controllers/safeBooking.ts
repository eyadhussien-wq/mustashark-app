...
import { bookingsTable, notificationsTable, usersTable } from "@workspace/db/schema";
...
function scheduledAtQatar(date: string, time: string) {
  ...
  const parts = new Intl.DateTimeFormat("en-US", { timeZone: "Asia/Qatar", year: "numeric", month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit", hourCycle: "h23" }).formatToParts(new Date(requested));
...
export const createBookingSafely = async (req: Request, res: Response) => {
  try {
    ...
    if (!scheduledStart) return res.status(400).json({ ok: false, error: "invalid_scheduled_datetime" });
