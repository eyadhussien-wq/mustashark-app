...
function isSupportedTimezone(value: string) { try { new Intl.DateTimeFormat("en-US", { timeZone: value }).format(); return true; } catch { return false; } }
function wallClockToUtc(date: string, time: string, timezone: string) { if (!isValidCalendarDate(date) || !isSupportedTimezone(timezone)) return null; const [year, month, day] = date.split("-").map(Number); const [hour, minute] = time.split(":").map(Number); const requested = Date.UTC(year, month - 1, day, hour, minute); const parts = new Intl.DateTimeFormat("en-US", { timeZone: timezone, year: "numeric", month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit", hourCycle: "h23" }).formatToParts(new Date(requested)); const values = Object.fromEntries(parts.map((p) => [p.type, p.value])); const zoneWallClock = Date.UTC(Number(values.year), Number(values.month) - 1, Number(values.day), Number(values.hour), Number(values.minute)); return new Date(requested - (zoneWallClock - requested)); }
...
export const createBookingSafely = async (req: Request, res: Response) => {
  try {
    ...
    const timezone = lawyer.schedulingTimezone;
    if (!timezone || !isSupportedTimezone(timezone)) return res.status(409).json({ ok: false, error: "lawyer_scheduling_timezone_not_configured" });
    const scheduledStart = wallClockToUtc(input.scheduledDate, input.scheduledTime, timezone);
    const scheduledEnd = wallClockToUtc(input.scheduledDate, formatTime(end), timezone);
    ...
    const booking = await db.transaction(async (tx) => {
      ...
      const [created] = await tx.insert(bookingsTable).values({ id: bookingId, serialNumber, clientId: authUser.id, lawyerId: lawyer.id, officeId: input.officeId || null, subject: input.subject, description: input.description || null, scheduledDate: input.scheduledDate, scheduledTime: input.scheduledTime, scheduledAtUtc: scheduledStart, scheduledTimezone: timezone, status: "pending", type: input.type, price: lawyer.hourlyRate ?? "0", paymentStatus: "pending" }).returning();
      await tx.insert(bookingTimeBlocksTable).values({ id: crypto.randomUUID(), bookingId, lawyerId: lawyer.id, scheduledDate: input.scheduledDate, startTime: input.scheduledTime, endTime: formatTime(end), scheduledAtUtc: scheduledStart, scheduledTimezone: timezone });
      ...
      await tx.insert(consultationEventsTable).values({ id: crypto.randomUUID(), bookingId, eventType: "CONSULTATION_CREATED", actorId: authUser.id, metadata: { initialState: "PAYMENT_PENDING", serialNumber, reference: serialNumber, price: lawyer.hourlyRate ?? "0", type: input.type, scheduledDate: input.scheduledDate, scheduledTime: input.scheduledTime, scheduledAtUtc: scheduledStart.toISOString(), scheduledTimezone: timezone } });
      return created;
    });
    return res.status(201).json({ ok: true, booking: { ...booking, reference: booking.serialNumber, timezone, scheduledEndTime: formatTime(end), scheduledStartAtUtc: scheduledStart.toISOString(), scheduledEndAtUtc: scheduledEnd.toISOString() } });
  } catch (error: any) {
