...
export const createBookingSafely = async (req: Request, res: Response) => {
  try {
    ...
    return res.status(201).json({ ok: true, booking: { ...booking, reference: booking.serialNumber, timezone: BOOKING_TIMEZONE, scheduledEndTime: formatTime(end), scheduledStartAtUtc: scheduledStart.toISOString(), scheduledEndAtUtc: scheduledEnd.toISOString() } });
  } catch (error: any) {
