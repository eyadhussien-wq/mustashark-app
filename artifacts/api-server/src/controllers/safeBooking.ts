...
const BOOKING_TIMEZONE = "Asia/Qatar";
function scheduledAtQatar(date: string, time: string) { if (!isValidCalendarDate(date)) return null; const [year, month, day] = date.split("-").map(Number); const [hour, minute] = time.split(":").map(Number); if (![year, month, day, hour, minute].every(Number.isFinite)) return null; const requested = Date.UTC(year, month - 1, day, hour, minute); const parts = new Intl.DateTimeFormat("en-US", { timeZone: BOOKING_TIMEZONE, year: "numeric", month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit", hourCycle: "h23" }).formatToParts(new Date(requested)); const values = Object.fromEntries(parts.map((p) => [p.type, p.value])); const zoneWallClock = Date.UTC(Number(values.year), Number(values.month) - 1, Number(values.day), Number(values.hour), Number(values.minute)); return new Date(requested - (zoneWallClock - requested)); }
async function resolveLawyer(lawyerId: string) { const [lawyer] = await db.select().from(usersTable).where(and(eq(usersTable.id, lawyerId), eq(usersTable.role, "lawyer"), eq(usersTable.accountStatus, "active"), isNull(usersTable.deletedAt))).limit(1); return lawyer; }
...
export const createBookingSafely = async (req: Request, res: Response) => {
  try {
    ...
    const booking = await db.transaction(async (tx) => {
      ...
      const [created] = await tx.insert(bookingsTable).values({ id: bookingId, serialNumber, clientId: authUser.id, lawyerId: lawyer.id, officeId: input.officeId || null, subject: input.subject, description: input.description || null, scheduledDate: input.scheduledDate, scheduledTime: input.scheduledTime, scheduledAtUtc: scheduledStart, scheduledTimezone: BOOKING_TIMEZONE, status: "pending", type: input.type, price: lawyer.hourlyRate ?? "0", paymentStatus: "pending" }).returning();
      await tx.insert(bookingTimeBlocksTable).values({ id: crypto.randomUUID(), bookingId, lawyerId: lawyer.id, scheduledDate: input.scheduledDate, startTime: input.scheduledTime, endTime: formatTime(end), scheduledAtUtc: scheduledStart, scheduledTimezone: BOOKING_TIMEZONE });
      ...
      await tx.insert(consultationEventsTable).values({ id: crypto.randomUUID(), bookingId, eventType: "CONSULTATION_CREATED", actorId: authUser.id, metadata: { initialState: "PAYMENT_PENDING", serialNumber, reference: serialNumber, price: lawyer.hourlyRate ?? "0", type: input.type, scheduledDate: input.scheduledDate, scheduledTime: input.scheduledTime, scheduledAtUtc: scheduledStart.toISOString(), scheduledTimezone: BOOKING_TIMEZONE } });
      return created;
    });
    const safeBooking = { id: booking.id, serialNumber: booking.serialNumber, clientId: booking.clientId, lawyerId: booking.lawyerId, subject: booking.subject, description: booking.description, scheduledDate: booking.scheduledDate, scheduledTime: booking.scheduledTime, scheduledTimezone: booking.scheduledTimezone, status: booking.status, type: booking.type, paymentStatus: booking.paymentStatus, createdAt: booking.createdAt };
    return res.status(201).json({ ok: true, booking: { ...safeBooking, reference: booking.serialNumber, timezone: BOOKING_TIMEZONE, scheduledEndTime: formatTime(end), scheduledStartAtUtc: scheduledStart.toISOString(), scheduledEndAtUtc: scheduledEnd.toISOString() } });
  } catch (error: any) {
