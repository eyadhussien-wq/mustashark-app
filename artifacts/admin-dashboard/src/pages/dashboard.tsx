...
import { formatCurrency, translateCountry, translateStatus, translateConsultationType } from "@/lib/formatters";
import { 
  Card, 
  CardContent, 
  CardHeader, 
...
export default function Dashboard() {
  ...
  const statusData = data.consultationsByStatus.map(s => ({
  ...
  return (
    <Layout>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4 animate-in fade-in slide-in-from-bottom-4 duration-500 stagger-1">
        ...
        <StatCard 
          title="مكاتب موقوفة" 
          value={data.suspendedOffices} 
          subtitle={`من إجمالي ${data.totalOffices} مكتب`}
          icon={AlertTriangle}
          alert={data.suspendedOffices > 0}
        />
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4 mt-6 animate-in fade-in slide-in-from-bottom-4 duration-500 stagger-2">
        ...
        <StatCard 
          ...
          value={formatCurrency(data.collectedCommission)} 
          icon={Wallet}
        />
        <StatCard 
          ...
          icon={AlertTriangle}
          alert={data.pendingCommission > 0}
        />
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-2 mt-6 animate-in fade-in slide-in-from-bottom-4 duration-500 stagger-3">
        ...
        <Card>
          <CardHeader>
            <CardTitle>حالة الاستشارات</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="h-[300px]">
              <ResponsiveContainer width="100%" height="100%">
      ...
      <Card className="mt-6 animate-in fade-in slide-in-from-bottom-4 duration-500 stagger-4">
        <CardHeader>
          <CardTitle>أحدث الاستشارات</CardTitle>
        </CardHeader>
        <CardContent>
                ...
                {data.recentConsultations.length === 0 ? (
                  ...
                  data.recentConsultations.map((consultation) => (
                    <TableRow key={consultation.id}>
                      <TableCell className="font-mono text-xs">{consultation.serialNumber || "-"}</TableCell>
                      ...
                      <TableCell>
                        <div className="text-sm">{new Date(consultation.scheduledDate).toLocaleDateString('ar-QA')}</div>
