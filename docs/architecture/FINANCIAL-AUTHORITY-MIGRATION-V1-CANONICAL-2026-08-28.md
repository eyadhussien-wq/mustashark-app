...
Financial safety is based on defense in depth, zero-trust inputs, least privilege, atomic state transitions, auditability, and reconciliation. The system must be designed so that a single malformed request, duplicated callback, concurrent request, compromised client, or partial provider failure cannot silently create a second financial effect or an untraceable balance change.

The implementation target is aligned with mature global payment-system practices, without copying any specific company's implementation.
...
Financial history must not be deleted or rewritten merely to make a test pass.
...
## 9. Concurrency and authorization
...
Authentication never replaces authorization. Every sensitive mutation must verify both the authenticated actor and the actor's authority over the specific booking/payment/financial resource.

## 10. Cancellation / No-show / Payout
...
- Production credentials and real customer/banking data must never be used in synthetic financial tests.

## 14. Legacy Policy Migration Rule
...
Old commercial-policy documents may remain only as historical/reference material and must be clearly marked **NOT ACTIVE**. They are not implementation authority for V1.
...
17. Full CI/Gate pass on the same branch head.

## 16. Synthetic Financial Scenario
...
**No production financial approval is implied by this document.**
