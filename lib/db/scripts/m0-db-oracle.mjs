...
export async function withIndependentOracle(run) {
  ...
  try {
    ...
    // The harness runs against a disposable *_test database that is torn down by CI.
    // Deliberately avoid DROP/TRUNCATE/DELETE here so the production DB mutation guard
    // remains fail-closed for every script under lib/db/scripts.
    await execution.end();
    await observer.end();
  }
}
