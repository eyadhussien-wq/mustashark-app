  ...
  try {
    ...
    const oracle = Object.freeze({
      ...
      async insertObservableMutation(requestId, actorId, mutationValue) {
        return insertObservableMutation(observer, schema, {
          requestId,
          actorId,
          mutationValue,
        });
      ...
      async readCurrentDbActor() {
        return readCurrentDbActor(execution);
      },
      async setLocalDbActor(actorId) {
        return setLocalDbActor(execution, actorId);
      },
    });
...
export async function insertObservableMutation(client, schema, observation) {
  ...
  const result = await client.query(
    ...
    [observation.requestId, observation.actorId, observation.mutationValue ?? 1],
  );
