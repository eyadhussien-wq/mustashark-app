...
export const disputesTable = pgTable(
  "disputes",
  {
    ...
    escrowAccountId: text("escrow_account_id")
      .notNull()
      .references(() => escrowAccountsTable.id, { onDelete: "restrict" }),
  ...
  (table) => ({
    ...
    escrowAccountIdx: index("disputes_escrow_account_id_idx").on(table.escrowAccountId),
