...
export const supersedeLegalDocumentSchema = z.object({
  fileName: z.string().trim().min(1).max(255), mimeType: z.string().trim().max(200).nullable().optional(), storageKey: z.string().trim().min(1).max(2000),
  content: z.string().min(1), title: z.string().trim().min(1).max(500), courtName: z.string().trim().max(500).nullable().optional(),
  caseNumberReference: z.string().trim().max(200).nullable().optional(), issuedAt: issuedAtSchema.nullable().optional(), metadata: legalDocumentMetadataSchema,
}).strict();
